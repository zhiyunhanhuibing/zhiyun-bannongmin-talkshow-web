		...
		<el-col :span="24" class="main">
			<aside :class="collapsed?'menu-collapsed':'menu-expanded'">
				...
				<el-menu class="el-menu-vertical-demo" @open="handleopen" @close="handleclose" @select="handleselect"
					...
					<template v-for="(item,index) in datafrom">
						<el-submenu :index="index+''" v-if="item.children">
							<template slot="title"><i :class="item.iconCls"></i>{{item.name}}</template>
							<el-menu-item v-for="child in item.children" :index="child.path" :key="child.path" v-if="item.children">{{child.name}}</el-menu-item>
						</el-submenu>
